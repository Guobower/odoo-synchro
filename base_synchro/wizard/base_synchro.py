...
class BaseSynchro(models.TransientModel):
    ...
    def synchronize(self, server, object):
        ...
        pool2 = pool
        request = self.env['res.request'].search([], limit=1, order='date desc')
        # dt = object.synchronize_date
        dt = request.date or object.synchronize_date
...
Exceptions:
    ...
    def upload_download_multi_thread(self):
        try:
            ...
            error_msg = str(e)
            _logger.warning('''Synchronize will be skipped because of error %s''', error_msg)
